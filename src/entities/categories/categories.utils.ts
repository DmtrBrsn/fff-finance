...
export const getIncExpStr = (cat?: Pick<Category, 'isIncome'>) => {
  if (cat===undefined) return '-';
  const isIncome = cat.isIncome
  return isIncome === false ? 'Expense' : isIncome ? 'Income' : '-'
}

export const orderCats = (cats: Category[])=> {
  return cats
  .sort(SortUtils.getNumSorting((c) => c.order, 'asc'))
  .sort(SortUtils.getBooleanSorting((c) => c.isIncome, 'desc'))
}

export const getDndReorderedCatUpdDocs = (cats: Category[], e: DroppableCollectionReorderEvent) => {
