...
export const Calculator = ({ value, setValue, close }: Props) => {
  ...
  const handleUpdateExpression = (value: string) => {
    ...
    else if (value.startsWith('00')) value = value.slice(1)
  ...
  return (
    <div className='calculator-container flex-col gap-3'>
      <div className='expression-container flex-col gap-1'>
        <TextField aria-label='Calculator' inputMode="none" onChange={handleUpdateExpression} value={expression}></TextField>
        <div className='result'>{result !== '' && `= ${result}`}</div>
      </div>
      <div className='flex-col gap-1'>
        <div className='flex-row gap-1 justify-sb'>
          {['7', '8', '9', '/'].map((char) => <Button size='l' key={char} onPress={() => handleUpdateExpression(expression + char)}>{char}</Button>)}
        </div>
        <div className='flex-row gap-1 justify-sb'>
          {['4', '5', '6', '*'].map((char) => <Button size='l' key={char} onPress={() => handleUpdateExpression(expression + char)}>{char}</Button>)}
        </div>
        <div className='flex-row gap-1 justify-sb'>
          {['1', '2', '3', '-'].map((char) => <Button size='l' key={char} onPress={() => handleUpdateExpression(expression + char)}>{char}</Button>)}
        </div>
        <div className='flex-row gap-1 justify-sb'>
          {['00', '.', '0', '+'].map((char) => <Button size='l' key={char} onPress={() => handleUpdateExpression(expression + char)}>{char}</Button>)}
        </div>
        <div className='flex-row gap-1 justify-sb'>
